				...
				// ARRAY
					$cutomization_pg_array = array(
						...
						array(
							...
							'fields'=>array(
								array('id'=>'evoau_001','type'=>'note','name'=>'Supported email tags that can be used within email message: <code>{event-edit-link}</code>,<code>{event-name}</code>,<code>{event-link}</code>,<code>{event-start-date}</code><br/>
									IF available only tags: <code>{submitter-name}</code>,<code>{submitter-email}</code>
									<br/>HTML codes can be used inside email message body.'),
...
?>
		...
		<div id="evoau_2" class="evcal_admin_meta evcal_focus">		
			...
			<div class="inside">
				...
				<?php endif;?>				
				<br/>
				<h4><?php _e('Guide to Capabilities:','eventon');?></h4>
