		...
		public function au_form_fields($var=''){
			...
			*/
			$event_fields = array(				
				...
				'event_location_select'=>array('Previous Saved Event Location Dropdown', 'evcal_location_select', 'locationselect','','evoAU_pseld'),
		...
		function save_form_submissions(){
			...
			if($created_event_id){	
				...
				// save location as taxonomy
					$location_name = !empty($_POST['evcal_location_name'])? $_POST['evcal_location_name']:
						( !empty($_POST['evcal_location_name_'])? $_POST['evcal_location_name_']: false );

					$location_address = !empty($_POST['evcal_location'])? $_POST['evcal_location']:
						( !empty($_POST['evcal_location_'])? $_POST['evcal_location_']: false );
					if($location_name){
						$this->set_new_term($location_name, 'event_location', $created_event_id);					
						$this->create_custom_fields($created_event_id, 'evcal_location_name', $location_name);
						$this->create_custom_fields($created_event_id, 'evcal_location', $location_address);
						$this->create_custom_fields($created_event_id, 'evcal_gmap_gen', 'yes');

						// location image
						if(!empty($_POST['evo_loc_img_id']))
							$this->create_custom_fields($created_event_id, 'evo_loc_img', $_POST['evo_loc_img_id']);				
					}
			...
			if(!$TERMEXIST){
				...
				if(!is_wp_error($NEWTERMID)){
					...
					if($taxonomy == 'event_location'){
						...
						$term_meta['location_address'] = (isset($_POST['evcal_location']))?$_POST['evcal_location']: 
							( !empty($_POST['evcal_location_'])? $_POST['evcal_location_']: null );
