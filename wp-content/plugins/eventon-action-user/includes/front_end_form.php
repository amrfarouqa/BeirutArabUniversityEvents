...
<div id='eventon_form' class='evoau_submission_form <?php echo $LOGINCHECK?'loginneeded':''; echo $LIMITSUB?' limitSubmission':'';?>'>
	...
	<?php 	wp_nonce_field( AJDE_EVCAL_BASENAME, 'evoau_noncename' );	?>
		...
		<?php
			// form subtitle text
			$SUBTITLE = ($atts && !empty($atts['stitle']))? $atts['stitle']:
		...
		<?php	
			...
			if($SELECTED_FIELDS!=false){
				...
				foreach($EACH_FIELD  as $__index=>$ff):
					...
					if(in_array($INDEX, $SELECTED_FIELDS) || in_array($INDEX, $this->au_form_fields('defaults_ar')) ){
						...
						// dont show name and email field is user is logged in
							if(is_user_logged_in() && ($INDEX=='yourname' || $INDEX=='youremail')){
								continue;
							}
						...
						switch($__field_type){
							// pluggable
								case has_action("evoau_frontform_{$__field_type}"):
									do_action('evoau_frontform_'.$__field_type, $field, $event_id, $default_val);
								break;
							...
							// default fields
								...
								case 'startdate':
									...
									echo "<div class='row'>
										...
										<p><input id='evoAU_start_date' type='text' readonly='true' class='evoau_dpicker req' name='event_start_date' placeholder='".eventon_get_custom_language($opt_2, 'evoAUL_phsd', 'Start Date', $lang)."' value='".$SD."'/><input class='evoau_tpicker req time' type='text' name='event_start_time' placeholder='".eventon_get_custom_language($opt_2, 'evoAUL_phst', 'Start Time', $lang)."' value='".$ST."'/></p>
									</div>";
								break;
								case 'enddate':
							...
							case 'color':
								echo "<div class='row'>
									<p class='color_circle' data-hex='".(!empty($EPMV['evcal_event_color'])? $EPMV['evcal_event_color'][0]:'8c8c8c')."' style='background-color:#".(!empty($EPMV['evcal_event_color'])? $EPMV['evcal_event_color'][0]:'8c8c8c')."'></p>
									<p class='evoau_color_picker'>
										<input type='hidden' class='evcal_event_color' name='evcal_event_color' value='".(!empty($EPMV['evcal_event_color'])? $EPMV['evcal_event_color'][0]:'8c8c8c')."'/>
							...
							case 'locationselect':
								...
								if ( ! empty( $locations ) && ! is_wp_error( $locations ) ){
									...
									echo '<p><select class="evoau_location_select">';
										echo "<option>".eventon_get_custom_language($opt_2, 'evoAUL_ssl', 'Select Saved Location', $lang)."</option>";
								    foreach ( $locations as $loc ) {
								    	$taxmeta = get_option("taxonomy_".$loc->term_id);
								       	echo "<option value='{$loc->term_id}' data-add='{$taxmeta['location_address']}' data-lon='{$taxmeta['location_lon']}' data-lat='{$taxmeta['location_lat']}' data-img='{$taxmeta['evo_loc_img']}'>" . $loc->name . '</option>';								        
								    }
								    echo "</select>
										<input type='hidden' name='evcal_location_name_' value=''/>
										<input type='hidden' name='evcal_location_' value=''/>
