...
class CactusThemeShortcodes{
	...
	function init(){		
		...
		if(!is_admin()){
			wp_enqueue_script( 'format-datetime-master', U_SHORTCODE_PLUGIN_URL . '/shortcodes/calendar-js/format-datetime-master/jquery.formatDateTime.min.js', array(), '', true );
			wp_enqueue_script( 'underscore-min', U_SHORTCODE_PLUGIN_URL . '/shortcodes/calendar-js/underscore/underscore-min.js', array(), '', true );
			wp_enqueue_script( 'jquery-migrate', U_SHORTCODE_PLUGIN_URL . '/shortcodes/calendar-js/jquery-migrate-1.2.1.min.js', array(), '', true );
		}
...
function calendar_data_json() {	
	if(isset($_GET['cal_json'])&& $_GET['cal_json']==1){
		...
		if($post_type!='u_course'){
			...
			$cat = $_GET['cat'];
			$ignore = $_GET['ignore'];
			if((isset($_GET['month-data'])&& $_GET['listview']==1) || isset($_GET['action_dt_stt'])){
			...
			if($the_query->have_posts()){
				...
				while($the_query->have_posts()){ $the_query->the_post();
					$image_src = wp_get_attachment_image_src( get_post_thumbnail_id(),'thumb_255x255' );
					$startdate = get_post_meta(get_the_ID(),'u_course_cat', true );
					if($startdate){
						$startdate_cal = gmdate("Ymd\THis", $startdate);
						$startdate = gmdate("Y-m-d\TH:i:s\Z", $startdate);// convert date ux
						$con_date = new DateTime($startdate);
