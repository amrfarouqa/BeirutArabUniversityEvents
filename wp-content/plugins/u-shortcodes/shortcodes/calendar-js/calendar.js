	...
	$(document).ready(function() {
		if(monthdata!='' && jsondata!='' && monthdata!=undefined && jsondata!=undefined) {
			...
			$("a#listview-calendar").click(function(){
				...
				var TimeOffset = new Date()
				var nTimeOffset = TimeOffset.getTimezoneOffset() / 60;
				json_source = $('#check-jsondata').val()+'?month-data='+month_update+'&cal_json='+cal_json+'&action='+action+'&post_type='+post_type+'&cat='+cat+'&tag='+tag+'&exclude='+exclude+'&ignore='+ignore+'&action_dt_stt='+action_dt_stt+'&listview='+1+'&nTimeOffsetToUTC='+nTimeOffset;			
				...
				$('#calendar-box button[data-calendar-nav=next].list-btt-rem').click(function(){
					...
					json_source= jsondata+'?month-data='+month_update+'&cal_json='+cal_json+'&action='+action+'&cat='+cat+'&tag='+tag+'&exclude='+exclude+'&ignore='+ignore+'&action_dt_stt='+action_dt_stt+'&listview='+1+'&nTimeOffsetToUTC='+nTimeOffset;
					getJsonDataList(json_source);
				});
