			...
			ob_start();
				...
				if(!empty($RSVP_LIST['y']) && count($RSVP_LIST['y'])>0){
					foreach($RSVP_LIST['y'] as $_id=>$rsvp){
						...
						?>
						<p><?php echo '#'.$_id.' '. $rsvp['lname'].', '.$rsvp['fname'].' <i>('.$rsvp['email'].( $phone? ' PHONE:'.$phone:'').')</i>';?><span data-id='<?php echo $_id;?>' data-status='<?php echo $_status;?>' class='checkin <?php echo ($_status=='checked')? 'checked':null;?>'><?php echo $_status;?></span><span><?php echo $rsvp['count'];?></span></p>
						<?php
					}
				}else{
		...
		function evoRS_generate_csv(){
			...
			if($entries->have_posts()):
				...
				while($entries->have_posts()): $entries->the_post();
					...
					echo $pmv['last_name'][0].",".$pmv['first_name'][0].",".$pmv['email'][0].",".
