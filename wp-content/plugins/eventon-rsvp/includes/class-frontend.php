			...
			if( !empty($emeta['evors_rsvp']) && $emeta['evors_rsvp'][0]=='yes'							
				...
				// show attending count 
					if(!empty($this->optRS['evors_eventop_attend_count']) && $this->optRS['evors_eventop_attend_count']=='yes'){
						...
						$output .= "<span class='evors_eventtop_section evors_eventtop_data'><em>".($yes_count>1?$yes_count.' ':'').'</em>'.$__count_lang."</span>";
					}

				// show remainging count 
					if(!empty($this->optRS['evors_eventop_remaining_count']) && $this->optRS['evors_eventop_remaining_count']=='yes'){
						// /print_r($object);
			...
			function GET_attendees_icons($eventID, $ri){
				...
				if(!empty($list['y'])){
					foreach($list['y'] as $feild=>$value){
						...
						$output .= "<span data-name='{$value['fname']} {$value['lname']} ". ($value['count']>1? '(+'.$value['count'].')':'' )."' >{$initials}</span>";
					}
				}
				return $output;
			}
