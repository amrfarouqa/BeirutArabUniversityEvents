...
class evosb_front{
		...
		public function template_loader( $template='', $file='') {
			...
			if(empty($file)){
				...
				if( !empty($post) && !empty($eventon_subscription_page_id) &&  $post->ID == $eventon_subscription_page_id ) {
					...
					wp_enqueue_style( 'evosb_page_styles',$eventon_sb->plugin_url.'/assets/evosb_subscriber_page.css');
					wp_enqueue_script( 'evosb_page_script',$eventon_sb->plugin_url.'/assets/evosb_subscriber_page.js',array('jquery'));
				}
			}
		...
		function send_email($args=''){
			...
			switch ($args['type']){
				...
				case 'newevent':
					...
					if(strpos($subject, '{event-name}')!==false){
						$subject = str_replace('{event-name}', $args['args']['event-name'], $subject);
					}
				break;
			}
			...
			//connect to eventon helper to send email					
				...
				$args_ = array(
					'to'=>$args['to'],
					'subject'=>$subject,
					'message'=>$this->emailpart_message($file, $args, $evolayout),
					'from'=>$this->emailpart_from_info(),
					'type'=> ( $args['type']=='newevent'? 'bcc':'normal')
				);
		...
		// manage subsctiption categories
			function manage_subscribe_categories(){
				...
				if($_saved_fields){	
					...
					echo "<button class='evosb_btn'>".evo_lang('Update Subscription')."</button></form>";
				...
				function _html_section_unsubscribe_btn(){
					...
					return "<p><a class='evosb_btn button' href='".$_link."'>".evo_lang('Unsubscribe')."</a></p>";
				}
	...
	// subscription button
		function subscription_button($atts){
			...
			if(!$only_logged || ($only_logged && is_user_logged_in())):
				...
				$this->print_scripts_on=true;				
				return '<a class="evosub_subscriber_btn evcal_btn"><em class="fa fa-envelope-o"></em> '. $buttonText.'</a>';
			endif;
		}
	...
	// ECHO form fields if set via settings
		function _html_additional_form_fields(){
			...
			if($_saved_fields){				
				...
				// input as lang field
					if($this->lang!='L1')
						echo "<input class='evo_lang' type='hidden' name='evo_lang' value='{$lang}'/>";
	...
	// front end styles and scripts
		function register_frontend_scripts(){
			global $eventon_sb;
			wp_register_style( 'evo_sb_styles',$eventon_sb->plugin_url.'/assets/evosub_styles.css');
			wp_register_script( 'evo_sb_script',$eventon_sb->plugin_url.'/assets/evosub_script.js');
		}
