 ...
 */
class Pix_WC_Tickets_List extends WP_List_Table {
    ...
    var $example_data = array(
            array(
                'ID'        => 1,
                'ticket_security_code'     => '300',
                'order_id'    => 'R',
                'order_status'  => 'Zach Snyder',
                'purchaser_name'        => 1,
                'purchaser_email'     => '300',
                'check_in'    => 'R',
            )
     ...
     ***************************************************************************/
    function __construct( $ticket_product_id ){
        global $status, $page;

        $this->ticket_product_id = $ticket_product_id;        
    ...
    function column_default($item, $column_name){
        ...
        switch($column_name){
            ...
            case 'order_status':
                ...
                echo '<span class="'.$order->post->post_status.'">'.ucfirst(str_replace('wc-', '', $item[$column_name])).'</span>';
    ...
    function get_columns(){
        $columns = array(
            ...
            'ID'     => 'Ticket ID',
            'ticket_security_code'    => 'Security Code',
            'order_id'  => 'Order ID',
    ...
    function get_table_values() {
        ...
        $ticket_id = $wpdb->get_results( "SELECT ID FROM $wpdb->posts WHERE post_type = 'pix_tickets' AND post_parent = '$this->ticket_product_id'", ARRAY_A );
        ...
        foreach ($ticket_id as $key => $value) {
            $order_id = get_post_meta( $value['ID'], '_order_id', true);
            $order = new WC_Order( $order_id );
            ...
            $table_values[] = array(
                'ID'                    => $value['ID'],
                'ticket_security_code'  => get_post_meta( $value['ID'], '_security', true),
                'order_id'              => $order_id,
