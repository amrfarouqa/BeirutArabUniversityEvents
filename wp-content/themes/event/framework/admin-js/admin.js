...
(function ($) {
	...
	window.displayMsg = function(msg, type){
		type = (typeof(type) == 'undifined') ? '' : type;
		$('#pix_event_details').find('#ticket-msg').html('<span class="'+ type +' msg">'+ msg +'</span>');
	};

	window.pixRemoveTicket = function(e) {
		...
		var $self = $(this),
			$btn = $('#pix-add-ticket'),
			woo_product_id = $btn.data('ticketid'),
			postid = $btn.data('postid'),
			nonce = $btn.data('nonce'),
			$msg = $('#ticket-msg'),
			$spinner = $msg.prev('.spinner');
		...
		$.ajax({
			...
			beforeSend: function(){
				...
				displayMsg( pixTicket.wait, 'info');
				$msg.show();
			},
			data: {
				action      	:   'pix_remove_ticket',
			...
			$msg.slideUp(300, function(){
				...
				console.log(response);
				if( response.woo_product_id == -1 ){
					$btn.text(pixTicket.btnTxtAdd).data('ticketid', response.woo_product_id);
					$self.data('ticketid', response.woo_product_id);

					$('input#ticket_title').val('');
					$('#ticket_desc').val('');
					$('input#_sku').val('');
					$('#_regular_price').val('');
					$('input#_stock').val(0);

					displayMsg( response.msg, 'success');
					$self.addClass('hidden');
				}else{
					displayMsg( response.msg, 'error');
					$self.removeClass('hidden');
				}		
				
				$msg.delay(300).slideDown(300);
			});
			...
			$self.on('click',pixRemoveTicket);
		});

	};

	window.pixAddTicket = function(e) {
		...
		var $self = $(this),
			woo_product_id = $(this).data('ticketid'),
			postid = $(this).data('postid'),
			nonce = $(this).data('nonce'),
			ticket_title = $('input#ticket_title').val(),
			ticket_desc = $('#ticket_desc').val(),
			_sku = $('input#_sku').val(),
			_regular_price = $('#_regular_price').val(),
			_stock = $('input#_stock').val(),
			$msg = $('#ticket-msg'),
			$spinner = $msg.prev('.spinner');
		...
		$.ajax({
			...
			$msg.slideUp(300, function(){
				...
				$self.text(pixTicket.btnTxtUpdate).data('ticketid', response.woo_product_id);
								
				displayMsg( response.msg, 'success');
				$msg.slideDown(300);

				$('#pix-remove-ticket').removeClass('hidden');
...
jQuery(document).ready(function($){
	...
	$( ".date_popup_from" ).datepicker({
      //defaultDate: "+1w",
      numberOfMonths: 2,
      minDate: 0,
      onClose: function( selectedDate ) {
        $( ".date_popup_to" ).datepicker( "option", "minDate", selectedDate );
      }
    });

    $( ".date_popup_to" ).datepicker({
      //defaultDate: "+1d",
      numberOfMonths: 2,
      onClose: function( selectedDate ) {
        $( ".date_popup_from" ).datepicker( "option", "maxDate", selectedDate );
      }
    });

    //Adding Tickets
	$('#pix-add-ticket').on( 'click', pixAddTicket);
	
	//Delete Ticket
	$('#pix-remove-ticket').on( 'click', pixRemoveTicket);
...
function showPageOptions(templateVal, $){
	var $options = $("#pix_page_options");



	if(templateVal == "page-template/page-event-tv.php" || templateVal == "page-template/page-event.php"){
		$options.hide(0);
